    ...
    return(
        <div className="orders-listing">
         <header>
            ...
            <div className="d-flex new_order justify-content-end mt-3">
                <button>
                    <span>
         ...
         <section className="filter-container mt-5 d-lg-flex align-items-center">
            ...
            <Select className="select-element" style={{width:"100%"}}>
               {status.map((s,i)=>(
                <Option key={`${i}-orders`} value={s}>{s}</Option>
               ))}
            </Select>
            </div>
         ...
         <Table 
           columns={columns} 
           dataSource={userOrders} scroll={{x:20}}
           onRow={(row)=>{
            return{
                onClick:()=>{
                    console.log(row)
